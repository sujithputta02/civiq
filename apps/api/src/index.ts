...
import { verifyFirebaseToken, verifyUserOwnership, enforceHttps, setSecureHeaders } from './middleware/auth';
import { validateSessionSecurity, detectSuspiciousActivityMiddleware, clearSession } from './middleware/security';
import { isZodError, getErrorMessage } from './types/errors';
import { requireAdmin } from './middleware/rbac';
import { secureResponseHeaders, sanitizeJsonResponse, preventResponseSplitting } from './middleware/output-encoding';
...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      connectSrc: ["'self'", 'https://generativelanguage.googleapis.com', 'https://api.tavily.com'],
      frameSrc: ["'none'"],
      objectSrc: ["'none'"],
    },
  },
 ...
 */
app.post('/api/v1/verify', verifyLimiter, verifyFirebaseToken, validateSessionSecurity, detectSuspiciousActivityMiddleware, async (req, res): Promise<void> => {
  try {
 ...
 * Security: Session hijacking protection
 */
app.post('/api/v1/chat', chatLimiter, verifyFirebaseToken, validateSessionSecurity, verifyUserOwnership, detectSuspiciousActivityMiddleware, async (req, res): Promise<void> => {
  try {
    ...
    const gemini = await getGeminiModule();
    const reply = await gemini.chatAssistant(userId, message, contextData || {}, explanationMode || '1m');
    res.json({ reply });
  } catch (error: unknown) {
 ...
 * Security: Session hijacking protection
 */
app.get('/api/v1/chat', verifyFirebaseToken, validateSessionSecurity, verifyUserOwnership, detectSuspiciousActivityMiddleware, async (req, res): Promise<void> => {
  try {
 ...
 */
app.get('/api/v1/admin/stats', verifyFirebaseToken, requireAdmin, async (req, res): Promise<void> => {
  try {
 ...
 */
app.get('/api/v1/admin/audit-logs', verifyFirebaseToken, requireAdmin, async (req, res): Promise<void> => {
  try {
    const userId = req.user?.uid || 'unknown';
 ...
 */
app.get('/api/v1/admin/security-events', verifyFirebaseToken, requireAdmin, async (req, res): Promise<void> => {
  try {
    const userId = req.user?.uid || 'unknown';
    const severity = req.query.severity as string | undefined;
    const limit = Math.min(parseInt(req.query.limit as string) || 100, 1000);

    await logAdminAction(userId, 'VIEW_SECURITY_EVENTS', '/api/v1/admin/security-events', { severity, limit });
