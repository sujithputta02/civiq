...
export async function verifyClaim(claim: string): Promise<VerificationResult> {
  ...
  try {
    ...
    const searchResponse = await tvly.search(claim, { searchDepth: 'basic', maxResults: 3 });
  ...
  const prompt = `
    ...
    ${searchContext}
    Analyze the following claim about the election process: "${claim}"
  ...
  try {
    ...
    await logToBigQuery('claim_verification', { claim, result: parsedResult });
    return parsedResult;
  } catch (error) {
...
export async function chatAssistant(userId: string, message: string, contextData: any, explanationMode: string = '1m'): Promise<string> {
  const API_KEY = process.env.GOOGLE_AI_API_KEY || '';
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${MODEL_NAME}:generateContent?key=${API_KEY}`;
  ...
  try {
    const tvly = tavily({ apiKey: process.env.TAVILY_API_KEY });
    const searchResponse = await tvly.search(message, { searchDepth: 'basic', maxResults: 3 });
    if (searchResponse.results && searchResponse.results.length > 0) {
      searchContext = 'Real-time context from the web:\n' + searchResponse.results.map((r: any) => `- ${r.title}: ${r.content} (${r.url})`).join('\n') + '\n\n';
    }
  } catch (searchErr) {
  ...
  const currentDate = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
  const userLocation = contextData?.location || 'Unknown';
  const systemInstruction = `You are an expert election assistant for Civiq. 
Today's date is ${currentDate}. 
User Context: ${JSON.stringify(contextData)}

CRITICAL INSTRUCTIONS:
1. The user's location is explicitly set to: ${userLocation}. 
2. You MUST ONLY provide election procedures, deadlines, and rules specific to ${userLocation}. 
...
5. You MUST tailor your output to this length constraint: "${explanationMode}".
...
${searchContext}`;
  ...
  if (finalReply !== 'Failed to generate response.') {
    ...
    await logToBigQuery('assistant_query', { userId, message, explanationMode });
  }
