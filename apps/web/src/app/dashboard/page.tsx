        ...
        <div className="mb-12">
          ...
          {showBetaMessage && (
            <div className="mt-4 p-4 rounded-2xl bg-blue-500/10 border border-blue-200 text-blue-700 text-sm flex items-center gap-3">
              <Bot size={20} />
              <span>
                <strong>Beta Feature:</strong> We are testing new AI models to improve your timeline
                accuracy. Thank you for being a Civiq pioneer!
              </span>
            </div>
          )}
        </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            {timeline.map((step, idx) => (
              <div key={step.id} className="relative flex gap-8">
                ...
                <GlassCard
                  className={`p-8 w-full group transition-all hover:scale-[1.01] ${
                  ...
                  <div className="flex flex-wrap gap-3">
                    ...
                    {step.status === 'COMPLETED' || completedStepIds.has(step.id) ? (
                      <RefractiveButton
                        size="sm"
                        variant="ghost"
                        className="gap-2 text-green-600 cursor-default"
                        disabled
                      >
            ...
            <GlassCard className="p-8">
              ...
              {stepsLeft === 0 ? (
                <p className="text-sm text-green-600 font-bold text-center">
                  🎉 You are fully election-ready!
                </p>
              ) : (
                <p className="text-sm text-secondary text-center">
                  Complete{' '}
                  <span className="font-bold">
                    {stepsLeft} more step{stepsLeft !== 1 ? 's' : ''}
                  </span>{' '}
            ...
            <GlassCard className="flex flex-col bg-blue-600 text-white border-none overflow-hidden h-[500px]">
              <div className="p-6 pb-4 border-b border-white/10 shrink-0">
                ...
                <p className="text-blue-100 text-sm mt-1">
                  Have questions about the process? I'm here to help.
                </p>
              ...
              >
                {messages.length === 0 ? (
                  <div className="text-center text-blue-200 text-sm mt-8">
                    Ask me anything about your election journey, deadlines, or voting procedures in{' '}
                  ...
                  messages.map((msg, idx) => (
                    ...
                    >
                      <div
                        className={`w-8 h-8 rounded-full shrink-0 flex items-center justify-center ${msg.role === 'user' ? 'bg-white/20' : 'bg-blue-800'}`}
                      >
