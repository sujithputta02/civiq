...
import { ShieldCheck, AlertCircle, CheckCircle2, XCircle, Search, ArrowLeft, Loader2 } from 'lucide-react';
...
export default function MythCheckPage() {
  ...
  const getStatusConfig = (classification: string) => {
    switch (classification) {
      case 'VERIFIED': return { icon: <CheckCircle2 className="text-accent-verified" />, color: 'border-teal-200 bg-teal-50/10', text: 'text-accent-verified' };
      case 'MISLEADING': return { icon: <AlertCircle className="text-accent-urgent" />, color: 'border-amber-200 bg-amber-50/10', text: 'text-accent-urgent' };
      case 'FALSE': return { icon: <XCircle className="text-accent-error" />, color: 'border-rose-200 bg-rose-50/10', text: 'text-accent-error' };
      default: return { icon: <ShieldCheck className="text-slate-400" />, color: 'border-slate-200 bg-slate-50/10', text: 'text-slate-600' };
    }
  };
    ...
    <div className="min-h-screen bg-[#F5F7FB] py-12 px-8">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mb-12 text-center">
          ...
          <p className="text-secondary">Paste any claim, screenshot text, or message about the election process to verify its accuracy.</p>
        </div>
        ...
        {result && (
          <GlassCard className={`p-8 animate-in fade-in slide-in-from-bottom-4 duration-500 ${getStatusConfig(result.classification).color}`}>
            <div className="flex items-start gap-4 mb-6">
              <div className="shrink-0 p-2 rounded-xl bg-white/80 shadow-sm">
                {getStatusConfig(result.classification).icon}
              </div>
              <div>
                <span className={`text-xs font-bold uppercase tracking-widest ${getStatusConfig(result.classification).text} mb-1 block`}>
        ...
        <div className="mt-12">
          <h4 className="text-sm font-bold text-secondary uppercase tracking-widest mb-4">Why verify?</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-4 rounded-2xl bg-white/30 border border-white/20 text-sm">
              ...
              <p className="text-secondary">Misinformation often targets marginalized groups with false info about IDs or dates.</p>
            </div>
            <div className="p-4 rounded-2xl bg-white/30 border border-white/20 text-sm">
              <p className="font-bold mb-1">Stay Confident</p>
              <p className="text-secondary">Knowing the real rules reduces polling-day anxiety and prevents procedural mistakes.</p>
            </div>
          </div>
        </div>
      </div>
